...
try {
    ...
    auto recv_cycle = [&]() {  // 主循环lambda,捕获所有变量
        using zmq_ctx = ZMQContext<1>; // 1 thread接收客户端消息
    ...
    auto log_cycle = [&]() {
        ...
        auto time_internal = conf.get<int>("config.time_interval");

        for (;;) {
            std::this_thread::sleep_for(std::chrono::seconds(time_internal));
